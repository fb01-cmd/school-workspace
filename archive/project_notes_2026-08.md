# Project Notes 아카이브 — 2026-08 전반

> `project_notes.md`에서 회전 이관된 원문이다 (`AGENTS.md` ④-1). **2026-08-01 ~ 2026-08-11 엔트리 309건.**
> 이관일 2026-08-12. 원문 그대로이며 편집하지 않는다.
> 인용은 **날짜 + 엔트리 제목**으로 한다 — 행 번호는 회전 때마다 어긋나므로 쓰지 않는다.
> 2026-08-12 이후 기록과 세션 종합 체크포인트는 [`../project_notes.md`](../project_notes.md)에 있다.
...
## [2026-08-11] E-3·4 UI 표적 리뷰 통과 → **Phase 9c-E 완결** ✅ + 다음 단계 결정 (Claude)

- **리뷰(f8a782a diff 실측 대조)**: 진입점 `aiEnabled !== false` 존재 숨김 ✓ / `enabled:false` 첫 응답 시 버튼 소멸 ✓ / 초안 전환 시 E-3·4 상태 초기화(E-1b F1 오귀속 교훈 동일 적용) ✓ / AI 표식 헤더 배지+하단 2중 ✓ / 에러 배너 통합 fail-visible ✓ / E-3·4 상호 disabled 교차 ✓ / 개발 용어·메타문구 0 ✓ / tsc·build(Claude 재실행) ✓.
- **문구 1건 Claude 직접 수정**: E-4 빈 결과 시 "현재 시간표가 데이터 기준으로 최적에 가깝습니다" → "AI가 뚜렷한 개선점을 찾지 못했습니다" — AI가 제안을 안 냈다는 사실을 플랫폼의 최적성 단정으로 승격하는 과대 주장 제거.
- **비차단 잔여**: ① 진단 버튼과 E-3·4 버튼 간 동시 클릭 미차단(읽기성 호출이라 무해, 무료 한도만 소모) ② 에러 배너가 다른 AI 액션 성공 후에도 잔존(각자 재시도 시 해제). 실기기 확인은 실사용 첫 회(증상 소멸 기준).
- **Phase 9c-E 완결** (E1 진단 · E2 정식화 · E3 설명 · E4 비평 — 서버부+UI 전부 구현·교차 검증 완료).
- **다음 단계 결정**: ① **F-2는 대기 유지** — 컴시간 NEIS 폴더 샘플 파일 확보 전 착수 불가(스키마·인코딩·가상 교사 표현 미정), 샘플 요청은 질문지에 포함돼 있음. ② **9월 질문지 작성을 지금(8월) 선행** — 전달·회수는 9월(학기 초 회피, 사용자 지시)이지만 문항 설계는 Fable-헤비 판단 작업이고 Max20이 8월 말 종료라 선행 확정 방침(메모리 기록)에 부합. 문항 원천 3계: 등록부 미비 하드 위반(특별교사 금지·연속수업 실무 규칙) / NEIS 샘플·가상 교사·복수교사 표기(F-2 열린 질문 3건) / S6·S7 소프트 가중치. ③ 그다음 8월 내 후보 = 쪽지 가을 스펙(선행 2순위).

### 재개 문구
- push 승인: *"푸시하자."*
- 질문지 작성(Claude): *"project_notes.md 마지막 체크포인트 읽어줘. 9월 질문지 문항 초안 작성해줘."*


## [2026-08-11] 9월 질문지 초안 v1 작성 ✅ ([`docs/september_questionnaire.md`](./docs/september_questionnaire.md))

- **구조**: [제1부] 전달용 본문(일과계 눈높이, 내부 용어·매뉴얼 조항 0 — ui-copy-rules 적용, 10~15분 분량) / [제2부] 내부 메모(문항→등록부·가중치·F-2 게이트 매핑 + 회수 후 처리 순서). **제1부만 전달할 것.**
- **문항 구성 5절**: ① 교사 교시 금지(배정/이동 구분 + 시간강사 출근 요일) ② 연속수업 ③ 복수교사 ④ 소프트 기준(S6 오전오후·S7 순배 중요도 + 자유 서술 — S7은 "신경 안 씀" 응답 시 미구현 확정 조항) ⑤ NEIS 샘플 파일 1개 부탁 + 가상 교사·복수교사 표기 2문항(F-2 열린 질문 3건 전부 수록).
- **전달·회수 = 사용자, 9월 중**(학기 초 회피). 회수 후: 등록부 등재 → 자동 작성 돌려 등록부미비 잔존 확인(E1 진단 활용) → 가중치 반영(Claude) → 샘플로 F-2 착수(Claude 스펙).
- **잔여(8월 내 Fable-헤비 후보)**: 쪽지 가을 스펙(선행 2순위) / 공동교육 계정 포털 차단 스펙(로드맵 §2 방향 확정분, 소형).
...
## [2026-08-11] 9월 질문지 v2 개정 ✅ — 프레임 모순 사용자 지적 → "이번 학기 복기" 정직화 + 실그리드 추출로 문항 축소

- **사용자 지적**: v1이 머리말은 "매 학기 공통 규칙"이라면서 1~3번은 학기 단위로 바뀌는 데이터를 물음 — 모순. 정리된 프레임 = 1~3번은 **이번 2학기 시간표를 짤 때의 조건 복기**(리허설용 등록부 부트스트랩, 신학기엔 차분 갱신), 4~5번만 학기 무관.
- **실측 전환**: 신규 `scripts/extract_questionnaire_candidates.ts`(읽기 전용 ~35 reads)로 2026-2 실그리드 역산 — **실과목 연속 블록 0건(창체·SLAT 2연속×학년 10회뿐)·복수교사 0건**. 2·3번을 백지 표 대신 "이렇게 읽었는데 맞습니까" 확인 문항으로 전환(부담 10분으로 축소). 2-2 신설 = 창체·SLAT 고정 일정 여부 → "맞다"면 연속 등록부가 아니라 일괄 배정 고정(솔버 순서 ①)으로 취급. 5-3은 실측 0건 반영해 과거 사례 문항으로 수정.
- **회수 후 갈림길(제2부 명기)**: 2-1·3 "맞다" 응답이면 연속·복수교사 등록부 **0건 확정**으로 종결 — 리허설 선행 조건이 1번(교시 금지)+4번(가중치)으로 줄어듦.
- **잔여**: 질문지 전달·회수(사용자, 9월) / 쪽지 가을 스펙·공동교육 차단 스펙(8월 내 후보).
...
## [2026-08-11] 공동교육 차단 §4 UI 리뷰 통과 + §5 오염 조사 완료 — 정리 대상 0건 ✅ (기능 잔여 = 사용자 활성화 1클릭)

- **§4 UI 리뷰(f1d16ce diff 대조)**: blockedOuPaths 로드·저장(merge:true 보존) ✓ / OUTreeSelector 재사용 ✓ / 등록 confirm 1회 ✓ / 중복 방지 ✓ / 섹션 번호 재정렬(일과표 9→10, 충돌 없음) ✓ / 문구 눈높이 ✓ — 수정 없이 통과.
- **§5 조사(`scripts/inspect_coop_user_docs.ts`, 읽기 전용 — users 전수 20 + GWS 대조)**: **공동교육 계정(ch_*)의 users 문서 오염 0건**(로그인 이력 자체가 없음), GWS 유령 0건 → **cleanup 스크립트 불요, §5 종결**. 조사 중 스크립트 결함 2건 자체 수정(mock의 ADMIN_EMAIL 제거로 사칭 조회 실패 → mock 미사용 명시 / "정리 대상 아님" 문자열 오탐 집계 → boolean 필드).
- **지나가다 본 것 1건**: mt02@(/학생/행사시연용계정)가 비학번 휴리스틱으로 teacher(승인 대기) 문서 보유 — 공동교육 아님(내부 시연용). 차단할지는 사용자 판단 사항(차단 원하면 §4 UI에서 해당 OU 추가만 하면 됨).
- **활성화 잔여(사용자 1클릭)**: blockedOuPaths 아직 비어 있음 — 환경설정 "9. 포털 접속 차단 조직단위"에서 **`/학생/공동교육과정(26)`** 추가·저장하면 즉시 작동(이 조작 자체가 §4 실기기 확인 겸용). 이후 ch_* 로그인 거부는 실사용 자연 확인.

### 재개 문구
- push 승인: *"푸시하자."*
- 쪽지 스펙(Claude): *"project_notes.md 마지막 체크포인트 읽어줘. 쪽지(사내 메신저) 스펙 작성해줘."*

## [2026-08-11] 쪽지(사내 메신저) 1단계 스펙 v1 확정 ✅ ([`docs/memo_spec.md`](./docs/memo_spec.md))

- **전제 고정(재론 방지)**: 순수 네이티브(메일 사본 철회 확정 반영)·교직원 전용·상태 단일 원본=플랫폼. 데모 포인트 = 보낸쪽지함 수신자별 읽음 실시간(§8 시연 시나리오 1줄이 완료 기준).
- **핵심 설계**: `memos/{domain}/items` 단일 문서 + `reads` 맵(이메일 키 — **FieldPath 필수**, dot-notation 금지) — 수신자별 fan-out 없이 읽음 표시·비용 최소. 읽기=클라 직독(onSnapshot, 복합 색인 2건)+규칙에서 수신·발신자 본인 검사 및 **isApproved 지역 강제**(전역 isTeacher() 미검사 §8 별건과 독립, 재검토 ① 충족). 쓰기=API 전용(`/api/memo` send·read).
- **수신자 검증**: 그룹은 서버 재귀 확장(클라 불신), 실존 대조는 users.list 전수 1~2콜 고정(getUser N회 금지), 학생 제외는 orgUnitPath `/학생` 실데이터 기준(휴리스틱 아님). 상한 300.
- **결정 사항**: 보존 기본 365일(`memoRetentionDays` 오버라이드) + expireAt 즉시 스탬프, 파기 크론 후행(**Vercel 크론 한도 소진 — 기존 통합 or GH Actions**). 푸시는 webpush.ts 재사용, tag 쪽지별, 페이로드 제목까지만, url `/`(뷰포트 분기 자연 통과 — 딥링크 안 함). /m엔 열람 전용 섹션(쓰기는 PC). 수신자 간 읽음 상호 노출은 수용 트레이드오프로 명기.
- **잔여**: §10 사용자 확인 3건(보존 365·발신 전 교직원·착수 시점 가을 유지) → 서버부 구현(Claude) → §4 화면(Antigravity) → 리뷰. 8월 내 Fable-헤비 선행분(질문지·공동교육 차단·쪽지 스펙)은 이것으로 소진 — 다음 대형 판단 작업은 9c-F(샘플 대기).
...
## [2026-08-11] 공동교육 차단 활성 실측 확인 ✅ + 환경설정 저장 UX 혼란 개선 결정 + mt02 종결

- **실측**: `settings.blockedOuPaths = ["/학생/공동교육과정(26)"]` (읽기 1건) — **차단 활성 작동 중**. 잔여는 ch_* 로그인 거부 실기기 확인뿐(자연 확인 가능).
- **UX 혼란 실증(사용자 신고)**: 차단 OU "추가"의 confirm 문구가 "차단됩니다. 정말 등록하시겠습니까?"라서 **즉시 적용으로 읽히지만 실제로는 화면 임시 목록에만 추가**되고, 하단 "매핑 설정 저장"을 눌러야 Firestore에 반영되는 구조(환경설정 탭 전 섹션 공통 일괄 저장). 사용자가 저장 버튼을 못 보고 완료로 오인.
- **개선 결정(Claude — 구조 유지, 신호만 정직화)**: ① confirm 문구를 "등록 목록에 추가합니다. 아래 '설정 저장'을 눌러야 실제로 적용됩니다."로 수정 ② 어떤 섹션이든 미저장 변경이 생기면 저장 버튼 옆에 "저장되지 않은 변경이 있습니다" 상시 배너 + 저장 버튼 강조, 저장 성공 시 해제 ③ 버튼명 "매핑 설정 저장"→"설정 저장"(매핑 외 전 섹션 저장이므로 오명) ④ dirty 상태에서 페이지 이탈 시 브라우저 확인창(beforeunload). 섹션별 즉시 저장 전환은 기각 — 전 섹션 저장 의미론 변경이라 리스크 대비 과함.
- **mt02@(행사시연용) 종결**: 사용자가 조만간 직접 삭제 예정 — 플랫폼 측 조치 없음.

### 재개 문구
- UX 개선(Antigravity): *"project_notes.md 마지막 체크포인트 읽어줘. 환경설정 저장 UX 개선 결정 ①~④ 대로 OUConfiguration.tsx 수정해줘."*
- 쪽지 §4 화면(Antigravity): *"project_notes.md 마지막 체크포인트 읽어줘. docs/memo_spec.md §4 대로 쪽지 화면(관리자 포털 'memo' 메뉴 + /m 열람 섹션) 구현해줘."*

## [2026-08-11] 환경설정 로드 경합 = 저장 덮어쓰기 위험 버그 수정 ✅ (Claude — 차단 OU "빈 화면" 신고의 근본 원인)

- **증상→진단**: 차단 OU 등록 후 재방문 화면에 "등록된 차단 조직단위가 없습니다" — **실데이터는 무사**(`["/학생/공동교육과정(26)"]` 재실측, 차단 작동 중). 원인 = `OUConfiguration` 초기화가 `[domain]` 의존 1회뿐이라 `schoolSettings` 구독 도착이 늦으면 전 섹션이 기본값으로 남음. 이 상태에서 "설정 저장"을 누르면 **차단 OU·OU 매핑·일과표 등 전 섹션을 기본값으로 덮어쓰는** 데이터 손실 구조(전 섹션 일괄 setDoc).
- **수정**: ① 수화(hydration)를 별도 useEffect `[domain, schoolSettings]`로 분리 — 구독 도착/변경 시마다 재수화 ② `handleSaveSettings`에 `schoolSettings` 미도착 시 저장 차단 + 버튼 disabled(신규 도메인 최초 저장이 막히는 트레이드오프는 데이터 보전 우선으로 수용·주석 명기) ③ OU 목록 fetch는 기존대로 `[domain]` 1회. tsc 0 / build ✅.
- **주의(배포 전)**: 이 수정이 배포되기 전까지는, 환경설정 화면에서 9번 섹션이 비어 보이면 **"매핑 설정 저장"을 누르지 말 것**(덮어쓰기 발생). 배포 후엔 새로고침만으로 저장된 칩이 정상 표시된다.
- UX 개선 결정 ①~④(직전 체크포인트)는 별건으로 유효 — 이 수정은 데이터 손실 차단, UX는 Antigravity 몫.

### 재개 문구
- push 승인: *"푸시하자."* (배포되어야 사용자 화면 검증 가능)

## [2026-08-11] hmnotice@ 정지 실사고 → 복구 ✅ + 목록 실종 버그 수정 + 보호 계정 가드 1차 구현 ✅ (Claude)

- **사고 경위(사용자 테스트)**: 플랫폼에서 hmnotice@ 일시정지 → 실측 결과 **GWS에서 실제 정지됨**(suspensionReason ADMIN — 보호 가드 부재 실증). "새로고침하니 원상복귀"는 착시(클라 users:all 5분/서버 캐시가 정지 전 스냅샷 표시). **admin SDK로 즉시 정지 해제 완료**(suspended=false 재확인) — 알림 발신 무중단.
- **검색 실종 원인**: users 라우트 list의 recentActionsCache "update" 패치가 `orgUnitPath` 포함 수정 시 요청 OU 목록에 없으면 계정을 목록에서 제거하는데, **`["all"]` 요청을 인지 못 해** 수정 직후 120초+캐시 TTL 동안 목록·검색에서 실종. → `includes("all")` 예외 추가로 수정.
- **보호 계정 가드 1차 구현**(로드맵 §2 항목 당일 구현, 상태 갱신 완료): 단일 소재지 = `workspace.ts` `deleteUser`(보호 계정 무조건 throw)·`updateUser`(suspended:true throw, 활성화는 허용) — 전 경로(개별·일괄·크론·전출) 자동 보호. 보조 가드 = users 라우트 4액션(눈높이 403+감사 로그, 일괄은 실패 항목으로 보고) + lifecycle `register_teacher_transfer`(삭제로 이어지는 큐 등재 차단). 잔여 확인 지점 = OU 이동·비번 초기화(미포함, 필요 시 후속).
- **검증**: tsc 0 / build ✅. 실기기 확인(선택): 사용자 전체관리에서 hmnotice@ 일시정지 재시도 → "보호 계정…" 안내가 뜨면 가드 실증(이제 안전).
- 참고: 정지 시 생애주기 큐 자동 등재는 **기존 문서가 있을 때만** 상태 갱신하는 구조라 hmnotice@ 큐 잔재 없음(확인).

### 재개 문구
- push 승인: *"푸시하자."*

## [2026-08-11] 보호 가드 유실 경위 복기 (사용자 지적) — "착수 대기" 분류 금지 원칙 신설

- **경위**: 오전 세션에서 보호 계정 3종 확정 시 하드 차단 기능이 로드맵 §2에 "착수 대기 — 사용자 지시 대기"로만 등재되고 어떤 체크포인트 잔여·재개 문구에도 오르지 않음 → 당일 hmnotice@ 정지 테스트가 실제로 먹히고서야 공백 발견(위 사고 엔트리). 사용자 인식은 "구현 지시했음" — 승인이 세션 경계에서 백로그로 격하된 핸드오버 유실.
- **원칙(양 에이전트 공통)**: **사용자가 확정한 보안·데이터 보호 결정은 "착수 대기"로 로드맵에만 두지 않는다** — 체크포인트 잔여·재개 문구에 등재해 다음 세션이 반드시 집게 하고, 소형이면 확정 당일 구현이 기본. (Claude 측은 영구 메모리에도 기록함)

## [2026-08-11] 보호 계정 가드 2차 보강 ✅ — 아이디 변경·OU 이동 차단 (사용자 확인 질문 계기)

- **사용자 지적**: "이름 변경·별칭 등록 등 다른 기능은 다 막힌 거 맞나? 아이디 변경도 크리티컬" — 실제로 1차 가드는 정지·삭제·전출 등재뿐이었고 **아이디(primaryEmail) 변경·OU 이동은 열려 있었음**(아이디 변경 = 알림 발신·DWD 사칭·인프라 연동이 주소 문자열에 묶여 있어 최악).
- **보강**: `workspace.ts` `updateUser` 단일 소재지에 추가 — ① primaryEmail 변경 차단 ② OU 이동 차단(수정 모달이 OU를 항상 동봉하므로 **현재 OU와 실제로 다를 때만** 차단, 보호 계정 한정 getUser 1회). **의도적 허용**: 활성화·이름·별칭·비밀번호(연동은 서비스 계정 키 기반이라 비번 무관). tsc 0 / build ✅.
- **3계정 실측 재확인**: hmnotice@·fb01@·admin@ 전부 suspended:false·주소·OU 원형 무사 (사용자 테스트 중 확인창은 취소 안내).
- **테스트 안내**: 배포 완료 후 정지/전출 등록/아이디 변경 시도 → 각각 "보호 계정…" 차단 문구가 뜨는 것이 정상(이제 무서워할 것 없음 — 가드 실증 겸용).
...
<!-- 2026-08-16 회전: live 236KB 초과(규칙 150KB)로 8/12~8/14 엔트리 이관 (원문 그대로, 상대 순서 보존) -->
