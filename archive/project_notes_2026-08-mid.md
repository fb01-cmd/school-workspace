# Project Notes 아카이브 — 2026-08 중반

> `project_notes.md`에서 회전 이관된 원문이다 (`AGENTS.md` ④-1). **2026-08-15 ~ 2026-08-17 엔트리 72건.**
> 이관일 2026-08-18. 엔트리 원문 그대로이며 편집하지 않는다.
> 주의: 원본 파일이 병합·덧붙이기로 시간순이 아니었어서, 이관은 **엔트리 블록 단위**로 했고
> 블록 다중집합 동일성(무손실)을 스크립트로 검증했다. 파일 내 순서는 원본에서의 상대 순서 그대로다.
