 ...
 *
 * [0] PDF 텍스트 층 추출 — 부서 7개 분할·자릿수 헤더 실재 (pdfjs가 pdftotext 근사를 내는가)
 * [1] 가명화 커버리지 — AI로 나가는 텍스트에 로스터 실명 잔존 0 (개인정보 게이트)
 * [2] 창체 결정론 파서 — 30반 담당 + 낡은 제목 함정(2025-1) 검출
 * [3] Gemini 추출(국어·수학) → 결정론 교차 검증 — 알려진 진실(수학 127 삼중 일치 등) 재현
...
async function main() {
  ...
  console.log(
    `[0] 추출·분할 ${chunks.length === 7 && headerOk ? "✅" : "❌"} — 부서 ${chunks.length}개(기대 7) · 반 번호 헤더 전 부서 실재 ${headerOk ? "✅" : "❌"}`
  );
  console.log(`    부서: ${chunks.map((c) => c.dept).join(" · ")}`);
  if (chunks.length !== 7 || !headerOk) failed = true;
  ...
  for (const target of ["국어", "수학"]) {
    const chunk = chunks.find((c) => c.dept.includes(target))!;
    ...
    if (target === "수학" && d.personalRows[0]?.cells[0]) {
