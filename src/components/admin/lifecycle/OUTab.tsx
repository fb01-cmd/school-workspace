...
export default function OUTab({ s, ud, onDone }: any) {
  ...
  const [steps, setSteps] = useState<{ label: string; status: SS }[]>([
    { label: `"3학년" OU (${g3 || "미설정"}) → "졸업생"`, status: "pending" },
  ...
  const run = async () => {
    ...
    try {
      const res = await callAPI(
        ...
        { grade1: g1, grade2: g2, grade3: g3, parentPath: parent },
        ud
      );
      setSteps((prev) =>
        prev.map((st, i) => {
          const found = res.steps?.find((r: any) => r.step === i + 1);
        ...
        <>
          ...
          {done ? (
            <div className="bg-green-50 border border-green-200 rounded-lg p-4 text-sm text-green-700 font-semibold">
              ...
              {onDone && (
                <button onClick={onDone} className="ml-3 px-3 py-1 bg-green-600 text-white text-xs rounded-lg hover:bg-green-700">
                  다음 단계로 →
                </button>
              )}
            </div>
