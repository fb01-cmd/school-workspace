...
discipline_config/{domain}                    ← 규정 단일 문서
  ...
  resetMarkers: { "1": timestamp, "2": timestamp, "3": timestamp }
    ← 학년별 수동 리셋(2026-07-25 사용자 확정: 자동 리셋 없음, 학생부가 학년 단위로 버튼 실행).
      리셋 = 기록 삭제가 아니라 마커 갱신. 회차 계산은 "마지막 리셋 이후 기록"만 집계.
      과거 기록은 이력으로 보존(불변 원칙 유지).
...
- **학생의 현재 단계는 저장하지 않고 계산**한다(마지막 리셋 이후 records + rules로 서버에서 산출). 규정이 바뀌면 재계산으로 자연 반영. 수동 개입은 stage_events의 manual 이벤트로 우선 적용.
- **규칙 모델은 "회차 → 처리 단계" 매핑**이다(2026-07-25 현행 시트로 확인): 같은 항목의 1회차·2회차·3회차가 각각 다른 처리 주체로 올라가는 구조. 예: 교복 1회차=담임, 2회차=생활지도교사, 3회차=생활교육위원회.

#### 초기 규정 시드 (현행 시트 기준 — 규정 편집기의 첫 데이터)

| 항목 | 1회차 | 2회차 | 3회차 |
|---|---|---|---|
| 생활지도(교복) | 담임 | 생활지도교사 | 생활교육위원회 |
| 흡연 | 1단계 | 생활교육위원회 | — |
| 휴대폰 | 1단계 | 생활교육위원회 | — |

- 단계 목록 시드: 담임, 생활지도교사, 1단계, 생활교육위원회 (순서·명칭 모두 편집 가능)
- 기록에는 비고 메모 필수 지원 (현행 시트의 "6/26 사복 착용(1교시)" 같은 날짜·상황 메모 대응)
...
- **파기 정책(2026-07-25 사용자 확정)**: 졸업/제적 시 해당 학생 기록 파기 — 현행 시트 운영과 동일한 기대. 구현은 lifecycle 크론의 계정 삭제 시점에 연동(계정 영구삭제 스텝에서 해당 학생의 discipline_records/stage_events 삭제). 감사 로그에는 "파기 실행" 사실만 남긴다(내용 미보존).
...
1. ~~생활지도 기록 파기 주기~~ → **확정: 졸업/제적 시 파기** (2026-07-25)
2. 마스터 시트를 둘 **공유 드라이브** 준비 (없으면 생성 필요 — 사용자가 GWS 관리콘솔에서)
   - ⚠️ 용도 명확화: 마스터 시트는 **시트로 남는 현황판(지필평가·모의고사)에 명단을 공급하는 통로일 뿐**, 생활지도와 무관. 생활지도는 시트 없이 100% 플랫폼 내 저장.
3. ~~현행 규정표 샘플~~ → **확보 완료: 초기 규정 시드 표 참조** (2026-07-25, 사용자 제공 시트 2장 분석)
4. 학년별 수동 리셋의 실행 권한을 누구에게 줄지 (기본안: manage_rules 권한 보유자 = 학생부)
