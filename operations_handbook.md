...
| 시기 | 할 일 |
|---|---|
| **월 1회 + 학기 경계** | **데이터 백업** — 아래 §6-1. 가장 중요하고 가장 잊기 쉽다 |
...
### 6-1. 데이터 백업 — 한 달에 한 번, 명령 두 줄

이 시스템의 **모든 데이터**(학생 명단, 생활지도 기록, 시간표, 쪽지, 작업 기록)는 구글 클라우드의 데이터베이스 한 곳에 있습니다. **지워지면 되돌릴 방법이 백업뿐입니다.**

```bash
# 1) 백업 만들기 — 1~2분
npm run backup

# 2) 그 백업이 정말 되돌릴 수 있는 것인지 확인 — 쓰기 없음, 안전
npm run backup:verify -- --from=~/school-backups/<방금 만들어진 폴더 이름>
```

- **어디에 저장되나**: `~/school-backups/` (내 컴퓨터. 저장소 안이 아닙니다)
- **얼마나 걸리나**: 1~2분. 시스템을 멈추지 않아도 됩니다
- **몇 개를 남기나**: **최근 3개 + 학기 경계 1개.** 나머지는 지웁니다
- **2)번이 왜 필요한가**: 백업 파일이 있어도 **정말 되돌아가는지는 해 봐야 압니다.** 이 명령이 그걸 확인합니다. `왕복 검증 통과`가 뜨면 안심해도 됩니다

> ⚠️ **이 파일에는 학생·교직원 개인정보가 그대로 들어 있습니다.**
> 메일·카톡·구글드라이브 등으로 **옮기지 마세요.** 내 컴퓨터 밖으로 나가면 그 순간 개인정보 유출입니다. 다른 사람에게 전달할 일이 생기면 먼저 상의하세요.

### 6-2. 장애 시나리오 6 — **"데이터가 지워졌어요"**

*(§5의 장애 시나리오 1~5에 이어지는 항목입니다)*

**증상**: 학생 명단이 통째로 비었다 / 생활지도 기록이 사라졌다 / 시간표가 날아갔다.

**순서대로 하세요.**

1. **일단 아무것도 더 하지 마세요.** 특히 "다시 만들어 보자"고 새로 입력하지 마세요 — 되돌릴 때 섞입니다.
2. **무엇이 언제 사라졌는지** 봅니다. 화면 왼쪽 **「작업 감사 로그」**에 누가 언제 무엇을 지웠는지 남아 있습니다.
3. **가장 최근 백업을 찾습니다**: `ls ~/school-backups/`
4. **먼저 되돌리지 말고 뭐가 되돌아가는지부터 봅니다** (실제로 쓰지 않습니다):
   ```bash
   npx tsx --env-file=.env.local scripts/restore_firestore.ts \
     --from=~/school-backups/<폴더> --all
   ```
   화면에 **"신규 N건 / 기존 M건"** 이 나옵니다. 신규가 사라진 데이터입니다.
5. **되돌립니다** — 사라진 것만 채우고 **지금 있는 것은 건드리지 않습니다**:
   ```bash
   npx tsx --env-file=.env.local scripts/restore_firestore.ts \
     --from=~/school-backups/<폴더> --all --apply
   ```
6. 화면에서 확인합니다.
...
- 기본은 **사라진 것만 채우기**입니다. 지금 있는 값을 백업 시점 값으로 **덮어쓰려면** 맨 뒤에 `--overwrite`를 붙여야 하는데, **그건 지금 있는 값이 사라집니다.** 확신이 없으면 붙이지 마세요.
- **계정 자체가 지워졌다면** 이건 다른 문제입니다 → §5 시나리오 3(구글 계정 20일 내 복원).
- 백업이 없거나 너무 오래됐으면 **더 손대지 말고 상의하세요.** 잘못 만지면 복구가 더 어려워집니다.

> 이 절차는 2026-08-14에 실제로 한 번 쓰였습니다 — 관리자 계정의 조직 정보를 정리하다 되돌린 건입니다. 그때 변경 전 89건이 자동으로 보관됐습니다.
