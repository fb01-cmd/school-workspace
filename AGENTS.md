...
## 4. 사용자 대상 차순위 지시 및 추천 모델 가이드

사용자는 비전문가이므로, 작업이 끝나거나 단계가 완료될 때 **반드시** 답변 마무리에 **"다음으로 어느 에이전트에게 무슨 지시를 해야 하는지 (추천 모델 포함)"** 바로 복사해서 쓸 수 있는 구체적 가이드를 추천한다.

### 모델(Engine) 선택 추천 기준 (토큰 경제학 최적화)
- **Claude / 심층 판단·스펙 설계·고난도 디버깅**: `Claude Sonnet 4.6 (Thinking)` 또는 `Gemini 3.1 Pro (High)`
- **Claude / 극단적 보안·되돌릴 수 없는 삭제·배포 최종 점검**: `Claude Opus 4.6 (Thinking)`
- **Antigravity / 코드 대량 생산·UI·DoD 빌드 검증**: `Gemini 3.6 Flash (Medium)` (기본 권장) 또는 `Gemini 3.6 Flash (High)`
- **Antigravity / 단순 오탈자·린트 수정**: `Gemini 3.6 Flash (Low)`

### 추천 예시 양식
- **Antigravity가 구현을 마쳤을 때**:
  > 💡 **다음 작업 지시 가이드**
  > - **대상 에이전트**: Claude
  > - **추천 모델**: `Claude Sonnet 4.6 (Thinking)`
  > - **복사용 프롬프트**: *"Claude, 방금 구현한 기능의 위험 지점을 표적 리뷰해줘."*
- **Claude가 스펙/판단을 내렸을 때**:
  > 💡 **다음 작업 지시 가이드**
  > - **대상 에이전트**: Antigravity
  > - **추천 모델**: `Gemini 3.6 Flash (Medium)`
  > - **복사용 프롬프트**: *"Antigravity, Claude가 정한 스펙대로 구현하고 E2E로 화면 검증해줘."*
