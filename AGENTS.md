...
| 작업 성격 | 선택 |
|---|---|
| 되돌릴 수 없는 작업(계정 삭제·복원, 마이그레이션), 보안 결정, 배포 직전 점검 | 최상위 추론 모델 (`Opus 4.8` 급) |
| 아키텍처 결정, 스펙 작성, 위험 지점 표적 리뷰, 2회 실패 버그 진단 | 최상위 추론 모델 (`Opus 4.8` 급) |
| 단순 확인, 커밋 정리, 짧은 질의응답 | 경량 모델 (`Haiku 4.5` 급) |

> Claude 계열은 버전이 자주 바뀐다. **작성 시점(2026-07-22) 기준 실존 라인업은 Claude 5 제품군 / Opus 4.8 / Haiku 4.5이며, 이 표의 모델명도 실제 드롭다운과 다르면 드롭다운을 따른다.**
