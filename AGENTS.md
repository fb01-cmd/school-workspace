...
> **집행 방법 (2026-07-24 추가)**: 위 1번 규칙이 문서에만 있고 실제로 지켜지지 않아, Phase 5.8에서 같은 파일(`workspace.ts`)을 동시에 편집하다 한쪽 수정이 조용히 사라지고 커밋된 main이 빌드 깨진 채로 남은 사고가 있었다. 이제부터 파일을 편집하기 전 **`project_notes.md` 최상단 "🔒 현재 작업 중 파일" 목록을 반드시 확인·갱신한다** — 다른 쪽이 이미 올려둔 파일이면 편집을 시작하지 않고 먼저 확인한다. 작업이 끝나면 그 목록에서 자기 항목을 지운다.
