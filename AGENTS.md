...
**벤치마크 근거 (2026-08-11 웹 검색 확인)**: Opus 5(2026-07-24 출시)는 Fable 5 대비 가격이 **정확히 절반**이면서 12개 공유 벤치마크 중 7개에서 앞선다(Frontier-Bench·GDPval-AA·OSWorld 2.0 등 지식노동·추론 계열). Fable 5가 앞서는 유일한 영역은 **코딩·초장기 자율 에이전트 작업**(SWE-bench Pro·CursorBench, 근소한 차이). 이 프로젝트에서 Claude가 하는 일(스펙·보안 리뷰·근본 원인 진단)은 판단 유형이라 Opus 5가 이기는 쪽 — 대량 자율 코딩은 애초에 Antigravity 몫이라 Fable 5의 우위 영역과 겹치지 않는다. 출처: [Anthropic 공식](https://www.anthropic.com/news/claude-opus-5), [Decrypt 벤치마크 비교](https://decrypt.co/374305/claude-opus-5-outscores-fable-5-most-benchmarks-half-price).

| 작업 성격 | 선택 |
|---|---|
| 되돌릴 수 없는 작업(계정 삭제·복원, 데이터 마이그레이션), 보안·개인정보 결정, 배포 직전 점검, 아키텍처 결정, 스펙 작성, 2회 실패 버그의 근본 원인 진단 | `Opus 5` — **기본 최상위** (판단 유형이라 Fable보다 유리하고 절반 가격) |
| 정말 코드 자체를 오래 자율적으로 다뤄야 하는 극히 드문 경우(대량 마이그레이션을 Claude가 직접 장시간 수행 등 — 이 프로젝트에선 거의 발생하지 않음, 발생 시에만) | `Fable 5` |
| 일반 코드 리뷰, 위험 지점 표적 리뷰, 문서·규칙 정리 | `Sonnet 5` |
| 단순 확인, 커밋 정리, 짧은 질의응답, 상태 조회 | `Haiku 4.5` |

기본값은 `Opus 5`로 두되, **판단이 필요 없는 기계적 작업이 예상되면 시작 전에 `Sonnet 5`나 `Haiku 4.5`로 낮춘다.** 대화가 시작된 뒤에 낮추면 이미 쓴 토큰은 돌아오지 않는다.
