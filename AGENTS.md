...
## 4. 작업 지시 수신 시 셀프 모델(Engine) 브리핑 규칙

에이전트(Antigravity 및 Claude)는 사용자가 새로운 작업 지시나 프롬프트를 전달했을 때, **작업을 시작하기 전(또는 첫 응답 상단에)** 지시받은 작업의 성격에 맞춰 **자신의 드롭다운 메뉴 중 어떤 엔진/모델을 선택하여 진행하는 것이 가장 효율적이고 안전한지 스스로 브리핑**한다.

### 작업 유형별 최적 모델 매칭 기준

#### 1) Antigravity 담당 영역 (생산·컴포넌트·UI·E2E 검증)
- **일반 생산 / UI 컴포넌트 구현 / E2E 화면 검증 / DoD 빌드 체크**: `Gemini 3.6 Flash (Medium)` (기본 권장) 또는 `Gemini 3.6 Flash (High)`
- **단순 오탈자 / 린트 수정 / 보일러플레이트**: `Gemini 3.6 Flash (Low)`
- **대규모 복잡한 파일 탐색 / 타입 부채 전면 개편**: `Gemini 3.1 Pro (High)`

#### 2) Claude 담당 영역 (판단·보안·스펙·표적 리뷰)
- **아키텍처 결정 / 스펙 작성 / 위험 지점 표적 리뷰 / 2회 실패 버그 진단**: `Claude Sonnet 4.6 (Thinking)` 또는 `Gemini 3.1 Pro (High)`
- **되돌릴 수 없는 삭제·복원 스펙 / 데이터 마이그레이션 / 배포 직전 점검**: `Claude Opus 4.6 (Thinking)`

### 브리핑 예시 양식 (작업 수신 후 첫 응답 상단에 출력)

```markdown
💡 **[작업 성격 분석 및 추천 엔진 브리핑]**
- **수행할 작업**: 사용자 관리 대시보드 컴포넌트 구현 및 E2E 검증
- **추천 엔진**: `Gemini 3.6 Flash (Medium)` (대량 생산 및 속도/토큰 소모 최적화)
- *(현재 선택된 모델과 다를 경우 드롭다운 변경을 안내)*
```
