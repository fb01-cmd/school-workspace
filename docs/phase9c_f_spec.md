...
> ## [2026-08-21 표기 정정] 「9월 샘플 확보 후」라는 조건은 낡았다 — 샘플은 2026-08-14에 들어왔다
>
> 이 문서는 §0·§6·§7에서 CSV 스키마 확정을 **9월 질문지 회수 이후**로 걸어 두고 있으나,
> 질문지와 나이스 샘플 파일은 **2026-08-14에 회수됐다.**
> 근거: `docs/phase9c_questionnaire_result_2026-08-14.md:4`(*"phase9c_f_spec.md §6 열린 질문이 닫힌다"*)·
> `:5`(*"5-1 부탁(나이스 파일 1개)도 함께 회수"*)·`:18`(5-2 답 = **파일에서 아예 빠진다**)·`:252`.
>
> **다만 낡은 것은 「회수 시점」 표기뿐이고, F-2 구현까지 끝났다는 뜻이 아니다.**
> 받은 파일이 **1학년 한 반분**이라 2·3학년 과목의 나이스 정식명은 아직 없고
> (`…questionnaire_result_2026-08-14.md:244`), 복수 교사 표기(5-3)는 실제 사례가 없어 확정을 미뤘다.
> 즉 **B1 차단 규칙은 그대로 유효**하며, F-2 구현 자체는 열려 있을 수 있다 —
> 열렸는지 여부는 이 문서가 아니라 [`STATUS.md`](../STATUS.md)에서 판단한다.

## 0. 범위 분할 — F-1 / ~~F-2 (9월 샘플 확보 후)~~ → **샘플은 2026-08-14 회수** (위 머리말)
...
## 6. F-2 — CSV 직렬화 (~~샘플 확보 후~~ → **샘플 2026-08-14 회수**, 스텁)
...
- ~~열린 질문(9월 질문지에 포함)~~ → **✅ 2026-08-14 회수 — 3건 중 2건 확정, 1건은 의도적으로 미확정**
  1. ~~컴시간 NEIS 폴더 샘플 파일 1개 (스키마·인코딩 확정)~~ → ✅ **받았다** (1학년 한 반분. `…result:4`·`:244`)
  2. ~~**창체·SLAT 등 가상 교사 수업이 컴시간 NEIS 파일에서 어떻게 표현되는가** (제외? 교사명 공란? 별도 코드?)~~
     → ✅ **답: 파일에서 아예 빠진다** (질의 5-2, `…result:18`)
  3. ~~복수교사 수업의 파일 내 표기(2행? 병기?)~~ → 🟡 **답이 "그런 수업이 없었다/모른다"** — 추측으로 굳히지 않고
     실제로 생기는 학기에 확인하기로 확정했다 (`…result` §5 5-3). 회수가 안 된 것이 아니라 **회수 결과가 이것**이다.
...
| F-2 | ~~사용자~~→Claude→Antigravity | ~~9월 질문지 샘플~~ ✅ **2026-08-14 회수 완료** → CSV 직렬화 스펙 확정 → 구현 |

> **[2026-08-21]** 위 표에서 사용자 몫(샘플 제공)은 끝났다. F-2의 앞 단계는 더 이상 사람을 기다리지 않는다 —
> 다만 2·3학년 과목명이 없어 B1 차단 규칙이 유효하므로, 착수 여부는 [`STATUS.md`](../STATUS.md)에서 판단한다.
