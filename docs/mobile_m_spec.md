# 모바일 전용 경로 `/m` 스펙 (2026-08-08, Claude) — ⛔ 폐지됨 (2026-08-21)

> **이 스펙은 2026-08-21 자로 폐지됐다. 아래 본문은 이력용이며, 지금 코드의 설명이 아니다.**
>
> 사용자 확정(*"폐지가자"*)에 따라 **반응형 단일화**로 전환했다 — 전 기기가 `/admin`으로 랜딩하고, `/m`은 `/admin`으로 넘기는 리다이렉트만 남았다(북마크·설치 앱 보호를 위해 경로는 지우지 않는다). 이 문서가 정의한 진입 분기(§2)·모바일 홈 구성(§4)은 **더 이상 존재하지 않는다.**
>
> - 폐지 근거·전환 비용 실측 = 로드맵 §2 「/m 폐지 — 반응형 단일화」
> - 전환 작업 기록 = 일지 2026-08-21 「/m 폐지 전환」
> - **잔재 없음 (2026-08-21 2단계 완료).** `src/components/mobile/` 3파일(`MobileMemoSection`·`MobileTasksSection`·`TodayTimetableCard`, 2,010줄)은 실기기 확인 통과 후 삭제했고 폴더도 사라졌다. 인계 당시 "4파일"로 적힌 목록에서 `src/app/m/page.tsx`는 빠진다 — 그 파일은 **리다이렉트 본체**이므로 살아 있어야 한다.
> - 이 문서를 되살릴 일이 생기면 삭제분은 `cf634a6` 직후 커밋 이전 이력에 있다.
