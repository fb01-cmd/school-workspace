...
> - 남은 잔재: `src/components/mobile/` **3파일**(`MobileMemoSection`·`MobileTasksSection`·`TodayTimetableCard`, 합 2,010줄)은 이제 어디서도 import되지 않는다 — **안정 확인 후 별도 커밋으로 삭제**한다(2단계). 인계 당시 "4파일"로 적힌 목록에서 `m/page.tsx`는 빠진다: 그 파일은 리다이렉트 본체가 됐으므로 **지우면 안 된다.**
