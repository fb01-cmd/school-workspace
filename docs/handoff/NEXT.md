...
9. **한 번에 한 과제만 한다.** 지금 차례는 **과제 V-2 하나**다. 커밋·핸드오버까지 마쳤으면 **거기서 멈추고 보고해라.**
10. **`project_notes.md`는 덧붙이기만 한다 (④-2).** 과제 U 커밋이 남의 엔트리 한 줄을 지웠다 — 커밋 전에 `git diff --numstat project_notes.md`의 삭제 열이 0인지 확인해라.

---

## 과제 V-2 — 과제 V 보수 2건 (검증 결과, 2026-08-22)

**기준 커밋: `e33f009`.** 과제 V 본체는 통과했다 — 대가 줄이 엔진 값을 그대로 쓰고(재계산·재정렬 없음), 1수 기보에도 총점 변화가 나오고, 「다른 감점이 늘어나지 않습니다」가 두 경우 모두 뜨고, 교사 격자 집기가 클릭과 같은 `handleTeacherCellClick`을 재사용하고, 스펙 §0-4도 정정됐다. 아래 **2건만** 보수한다.

### V2-1. 「➕ 시간표 추가」 버튼이 좁은 화면에서 아직도 소리 없이 사라진다

`DraftAutoTab.tsx:3165`에 `hidden lg:flex`가 남아 있다. **이게 사용자가 신고한 *"아이콘도 사라져"* 의 정체다.**

**내 지시서 결함이다** — V-3에서 「직접 조정 토글」만 짚어서 이 버튼이 빠졌다. 토글은 제대로 고쳤다.

- 토글에 한 것과 **똑같이** 바꿔라: 숨기지 말고 **흐린 비활성 + 이유 안내**(`disabled` + `title`).
- 그리고 **`hidden lg:` 를 파일 전체에서 grep 해 0건인지 확인하고 보고해라.** 이 패턴이 세 번째로 빠져나갔다(토글 → 배정하기 → 시간표 추가). 남은 게 또 있으면 같이 고쳐라.

### V2-2. 안내 문구에 `1024px`가 그대로 나온다

사용자에게 보이는 두 곳:
- `:337` 말풍선 — *"화면 폭이 좁아져(**1024px 미만**) 직접 조정 모드가 해제되었습니다"*
- `:3135` 툴팁 — *"직접 조정 모드는 화면 폭 **1024px 이상**(넓은 화면)에서 사용할 수 있습니다"*

**픽셀 수는 개발자 단위다.** 선생님이 읽고 무엇을 해야 할지 아는 말로 바꿔라 — 예: *"화면이 좁아져 직접 조정이 꺼졌습니다. 창을 넓히면 다시 쓸 수 있어요."* / *"창을 넓히면 쓸 수 있어요."* (`:3108`은 주석이라 그대로 둬도 된다.)

**완료 확인**: tsc·build·check:ui·`check_ui_removals`(기준 `e33f009`) + selftest 4종 유지 + ⓐ`hidden lg:` grep 결과 전문 ⓑ바꾼 문구 전문. **핸드오버 ④ 기재**(덧붙이기만).
