...
9. **한 번에 한 과제만 한다.** 지금 차례는 **과제 S-3 하나**다. 커밋·핸드오버까지 마쳤으면 **거기서 멈추고 보고해라.** S·T·U는 검증을 통과한 뒤 Claude가 다시 지시한다 — 미리 손대지 마라(같은 파일이라 겹치면 검증이 불가능해진다).

---

## 과제 S-3 — 한 줄 보수 (검증 결과, 2026-08-22)

**기준 커밋: `239f908`.** 과제 S-2는 셋 다 제대로 됐고, 부탁하지 않은 실행취소·다시실행 가드까지 훑어서 붙인 것도 맞다. **`handleOpen`에 초기화를 넣기로 한 선택도 옳다** — 초안 목록에서 다른 초안을 열 때까지 같이 해결됐다. 다만 그 선택이 **부작용 하나**를 만들었다. 이건 그렇게 하라고 권한 내 지시서 탓이다.

### S3-1. 충돌 안내 문구가 뜨자마자 지워진다 (신규 회귀)

`DraftAutoTab.tsx:1243`에서 「다른 창이 먼저 수정했습니다…」를 띄운 직후 `handleOpen`을 부르는데, `handleOpen`의 진입부(`:759`)가 `setBlockedBubble(null)`을 한다. **나중 것이 이긴다 — 사용자는 판이 왜 갑자기 바뀌었는지 못 본다.** S-2 전에는 `handleOpen`이 이 값을 안 건드려서 문구가 살아 있었다.

- 처방(어느 쪽이든 됨, 고른 쪽을 보고에 적어라):
  ⓐ `handleOpen`에서 `setBlockedBubble(null)`을 빼고, 초안 목록에서 여는 경로(`:4717` 부근)에서만 지운다
  ⓑ `handleOpen`에 「지울지 말지」 인자를 하나 받아 409 경로에서는 유지한다
  ⓒ 409 경로에서 `await handleOpen(...)` **뒤에** 문구를 다시 세운다
- 어느 쪽이든 **409가 났을 때 문구가 화면에 남아 있어야** 한다.

### S3-2. 같이 정리 — 409 분기의 중복 7줄

`:1246-1252`의 초기화 7줄은 이제 `handleOpen`이 같은 일을 하므로 **죽은 줄**이다. 지워라. (S3-1을 ⓒ로 고른다면 순서를 살펴 지워라.)

**완료 확인**: tsc·build·check:ui·`check_ui_removals`(기준 `239f908`) + selftest 3종 유지 + S3-1을 ⓐⓑⓒ 중 무엇으로 했는지와 그 근거(file:line). **핸드오버 ④ 기재.**

---

## 과제 T — 그리드 고정(📌)·시간표 추가 + 셀 높이 상시 균일화 (사용자 실기기 피드백 2026-08-22)

**착수 조건: 과제 S-3을 먼저 끝내라(같은 파일 — 순차). R·R-2는 종결됐다.** 배치 원본 = `docs/timetable_manual_move_spec.md` §2-2 개정판·§2-3 승격 조항(이 커밋에 반영됨).
...
**착수 조건: 과제 S-3·T를 먼저 끝내라(같은 파일 — 순차). R·R-2는 종결됐다.** 배치 원본 = `docs/timetable_lookahead_spec.md` §3·§4.
