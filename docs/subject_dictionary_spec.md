...
**신학기 도태 (2026-08-17 실기기 확인 중 확정)**: 그리드가 아직 없는 학기의 관문 저장에서는, 승계 사전 항목 중 **이번 배정표가 쓰지 않는 것을 정리**한 뒤 확정을 반영한다(`pruneSubjectsToReferenced`). 근거: 안 쓰는 작년 표기(분반 쪼개기 "논술A/B"의 약칭)가 올해 정식명("논술")의 신규 등록을 표기 충돌로 막은 실사고. 사용자 원칙 원문: *"작년 기준으로 관성적으로 하는 걸 최대한 줄이고 올해 받은 리소스만으로."* 운영·채택 학기에는 적용하지 않는다(살아 있는 그리드가 참조하는 항목 보호).
