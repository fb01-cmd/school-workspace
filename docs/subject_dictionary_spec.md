...
**교차 검증 (2026-08-17)**: 이 원칙은 컴시간의 조치 방식과 동형이다 — 정식↔단축 쌍은 기초자료에서 사람이 선언, 과목 쪼개기 금지, NEIS 대조 불일치는 빈칸(시끄러운 실패)으로 노출하고 사람이 정정. 상세 = [`9c_research_notes.md`](./9c_research_notes.md) §5.

**신학기 도태 (2026-08-17 실기기 확인 중 확정, 같은 날 조건 정정)**: **운영 학기가 아닌 학기**(초안·미래 학기)의 관문 저장에서는, 승계 사전 항목 중 **이번 배정표가 쓰지 않는 것을 정리**한 뒤 확정을 반영한다(`pruneSubjectsToReferenced`). 근거: 안 쓰는 작년 표기(분반 쪼개기 "논술A/B"의 약칭)가 올해 정식명("논술")의 신규 등록을 표기 충돌로 막은 실사고. 사용자 원칙 원문: *"작년 기준으로 관성적으로 하는 걸 최대한 줄이고 올해 받은 리소스만으로."* 운영 학기에는 적용하지 않는다(판정 원본으로 살아 있는 그리드가 참조하는 항목 보호). ~~1차 조건 "그리드 없는 학기"~~는 실측에서 틀렸다 — 학기 전환이 참고용 그리드를 초안 학기에 복사해 오므로(2027-1 실측), 조건은 `activeTermId ≠ 대상 학기`다.

**같은 저장 안의 표기 자기 충돌 (2026-08-17 실사고)**: 배정표 「인공지능 기초」와 이동수업 현황 「인공지능기초」가 둘 다 create로 오면, 정식명이 정규화(공백 무시) 동일한 두 번째 create는 오류가 아니라 **먼저 만든 항목의 별칭으로 자동 합류**한다 — 추측이 아니라 정규화 동일성이므로 안전. 약칭·별칭 자리와의 충돌은 오류 유지.
...
- **[2단계 확장 2026-08-17]** 관문 목록에는 배정표 행 외에 **이동수업 현황 문서 표기(`fromSimulStatus`)와 대상 학기 등록부(이동수업·특별실)의 과목 표기(`fromRegistry`)**도 오른다 — 승계 등록부가 작년 표기("인공Ⅱ")를 쓰는 실물을 여기서 별칭으로 확정해야 소비자 정확 일치가 성립한다. 두 부류 모두 비차단(건너뛰기 출구)이며, **행 유래가 아닌 항목의 new 상태는 자동 새 등록으로 선반영하지 않는다**(등록부 표기를 실수로 독립 과목으로 등록하는 사고 방지 — 기본값은 건너뛰기).
...
- **2단계 (관문 UI 배포 + 운영 학기 별칭 확정 후)**: 폴백 제거 — subjectMatches·subjectStemLoose의 소비자 노출을 닫고 suggestCandidates 내부로만 남긴다. 제거 조건은 "운영 학기에서 폴백 고지 0건"이며, 제거는 별도 커밋으로 한다(회귀 시 되돌리기 쉬운 단위).
  - **[2단계 완결 ✅ 2026-08-17]** 감사 전 학기 0건 확인 후 **런타임 판정 3곳의 폴백 제거**: buildSimulMatcher(그리드 스탬프·H7 검사 경유)·venueProbe(특별실 조회)·솔버 동시수업 구성원 수집 — 이제 정규화 동일 또는 사전 정확 일치(별칭 포함)만 판정한다. `subject-loose-bind` 고지도 함께 소멸. **관문 쪽 보조는 유지**: suggestCandidates(후보 제안)·assembleHoursRows 태그 힌트·이동수업 현황 대조(클러스터링·고지)는 "제안·고지 엔진" 역할이라 4cac058 확정("후보 제안 엔진으로만 강등·재사용")의 그 자리다 — 이들은 판정을 확정하지 않고 사람 확인(관문·확인 목록)으로 이어진다.
  - **[조건의 단일 점검 도구 2026-08-17]** `npx tsx --env-file=.env.local scripts/audit_subject_loose_binds.ts` — 학기별로 등록부 표기 × 그룹 범위 수업 이름 쌍 중 "사전 판정 불능 + 느슨 매칭이 결정"인 쌍을 전수 나열한다. **전 학기 0건이 제거 조건.** 첫 실측(2026-08-17): 운영 2026-2 = 0건, 초안 2027-1 = 14건(전부 승계 등록부의 작년 표기 — 인공Ⅱ/Ⅲ·지구·수탐A/B·논술B). 감사에서 「수탐A」표기↔「수탐B」수업 같은 **교차 오연결 위험**도 실측됨 — 폴백 제거가 안전성 개선인 이유.
